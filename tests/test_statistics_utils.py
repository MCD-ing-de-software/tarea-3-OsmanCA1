...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        resultado = utils.moving_average(arr, 2)
        npt.assert_allclose(resultado, expected)  
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        self.assertAlmostEqual(resultado.mean(), 0.0)  
        self.assertAlmostEqual(resultado.std(), 1.0)   
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        utils = StatisticsUtils()
        arr = [2, 4, 6]
        resultado = utils.min_max_scale(arr)
        expected = np.array([0.0, 0.5, 1.0])
        self.assertAlmostEqual(resultado.min(), 0.0)
        self.assertAlmostEqual(resultado.max(), 1.0) 
        npt.assert_allclose(resultado, expected) 

...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        """
        dataframe2 = make_sample_df()
        cleaner = DataCleaner()
    
        with self.assertRaises(KeyError):cleaner.drop_invalid_rows(dataframe2,["Noexiste"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        nombre = dataframe.loc[0, "name"]
        ciudades = dataframe.loc[0, "city"]

        self.assertEqual(dataframe.loc[0, "name"],nombre)
        self.assertEqual(resultado.loc[0, "name"],nombre.strip())
        self.assertEqual(resultado.loc[0, "city"],ciudades)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        dataframe = make_sample_df()
        cleaner = DataCleaner()
        resultado = cleaner.remove_outliers_iqr(dataframe,"age", 1.5)
